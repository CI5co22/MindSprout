...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, zIndex = 50 }) => {
  ...
  return (
    <div 
      className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200"
      style={{ zIndex }}
    >
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
        <div className="px-8 pt-8 pb-4 flex justify-between items-center">
          ...
          <button onClick={onClose} className="p-2 rounded-full bg-slate-100 text-slate-400 hover:text-rose-500 transition-colors">
